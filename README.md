...
There are 5 high-level operations that can be performed:
* Register User
* Get User Details
...
* <i>Prepare/Voting Phase:</i>
    ...
    * All Participants Prepare upto the Transaction and sends back reply message (either Agreement(yes) or Abort(no) votes) to the Coordinator

* <i>Commit/Completion Phase:</i>
    * Coordinator can receive either Agreement or Abort from each Participant.
    * If the Coordinator received agreement message from all the Participants, it is Ready:
        * Coordinator sends the Commit message to all the Participants
        * All Participants commits transactions & sends acknowledgement to the coordinator
    * If the coordinator received abort message from any one participant, it has Failed:
        * Coordinator sends a Rollback message to all the Participants
        * All Participants undo the transaction & sends acknowledgement to the Coordinator
...
‘Coordinator node’ and ‘Worker node’ are the communicating objects. The interaction between Coordinator and Worker node is a very crucial part of the 2 phase protocol. The Coordinator node communicates with the Worker nodes regarding the actions that the Worker nodes have to perform on their respective Files. The Worker nodes have to inform the Coordinator nodes regarding failure or success of its operations on its Files. However tight coupling between the interaction of these nodes will require constant modification of one file in case of modification of the interface of the other file. Hence, the mediator pattern helps to reduce coupling between these objects.
...
‘Worker node’ performs file operations on the File objects using interfaces provided by DBFile. In cases of failure of ‘Worker nodes’, the File objects have to be rolled back to their previous state. Therefore, ‘Worker node’ stores the current state of ‘File objects’ in the form of logs before performing any update operations on them. However, if the implementation of ‘Worker nodes’ depends on the structure of state of ‘File objects’, constant modification has to be made in the way logs are stored and processed whenever structure of ‘File objects’ are changed. Therefore, ‘File object’ state is remains same and hidden (‘ void *’ in our case). Any rollbacks to the operations will require the ‘Worker node’ to pass the previous state (void*) to the DBFile object which will take care of the rollback operation.
...
* 2PC is a Blocking protocol hence it ends up in delaying progress in the long run & does not utilize resources properly during this time (This can be overcome by implementing timeouts).
